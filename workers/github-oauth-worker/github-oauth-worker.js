...
export default {
  async fetch(request, env) {
    ...
    // Requires both app secret and write secret - same bar as github-ops-worker.
    if (url.pathname === "/refresh") {
      ...
      if (!checkWriteAuth(request, env)) {
        return new Response(JSON.stringify({ error: "Write secret required" }), {
          status: 401, headers: { "Content-Type": "application/json", ...CORS },
        });
      }
